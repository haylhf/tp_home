...
function clearVipUIData() {
	...
	for (let item of _this.vipsignList) {
...
export default {
	...
	methods: {
		...
		getAvailableIndexVip: function () {
			...
			if (index == -1) {
				if (_this.vipsignList.length > 0) {
					index = _this.vipsignList[0].index;
					_this.vipsignList.splice(0, 1);
				}
			}
			return index;
	...
	mounted: function () {
		...
		currentInterval = setInterval(function updateTime() {
			if (_this.isVip) {
				...
				try {
					let delaytime = 15;
					if (_this.vipsignList.length >= MaxCount - 2) {
						delaytime = 5;
					}
					else if (_this.vipsignList.length >= MaxCount - 5 && _this.vipsignList.length < MaxCount - 2) {
						delaytime = 10;
					}
					if (_this.isLoadisLoading) {
						return;
					}
					for (let i = 0; i < _this.vipsignList.length; i++) {
						if (_this.vipsignList[i].updateTime) {
							let dtime = new Date() - _this.vipsignList[i].updateTime;  // 计算时间差
							let diffTimes = Math.floor(dtime / 1000); //算出总的秒数差值
							if (diffTimes >= delaytime) {//delaytime 秒内没有人刷卡，则部门变小回退到原来位置
								if (_this.isLoading) {
									break;
								}
								_this.playAnimationToResetVip(_this.vipsignList[i]);
							}
						}
					}
				} catch (ex) {
